...
const computeAvailableCells = (val: boolean, game: Game, cell_coords : {x:number, y:number}) : void => {
...
canvas.addEventListener('mouseup', (event : MouseEvent) => {
    ...
    if (event.button === 0 && game.selecting && (game.selectedCell?.x !== cell_coords.x || game.selectedCell?.y !== cell_coords.y) ) { // If dragged to a different cell
