...
const takeEnPassantAvailableCell = (game, currX, currY) => {
    const board = game.board;
    const currPiece = board[currY][currX].piece;
    const forwardDir = currPiece.color === Color.Black ? 1 : -1;
    if (game.getLastMove() === null)
        return [];
    const lastPieceMoved = game.getLastMove().pieceMoved; // "!" to fix
    const out = [];
...
const removePiece = (game, x, y) => {
    const board = game.board;
    const oldCell = board[y][x];
    const piece = oldCell.piece;
    if (piece.pieceType === PieceType.None || piece.color === Color.None)
        return;
    oldCell.piece = new Piece();
};
const movePiece = (game, oldX, oldY, newX, newY) => {
    clearMessage();
    const board = game.board;
    ...
    const move = new Move(game, oldX, oldY, newX, newY);
